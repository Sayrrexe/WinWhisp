...
- Окно настроек, открытое из трея, теперь сразу становится активным окном Windows — dropdown'ы (выбор модели, устройства, языка, compute type) и кнопка «Установить» больше не игнорируют первый клик из-за того, что фокус остаётся за прежним приложением.
